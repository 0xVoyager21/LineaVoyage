...
class Account:
    def __init__(self, id, private_key, proxy, network):
        ...
        if self.proxy != None:
            self.w3 = Web3(Web3.HTTPProvider(RPC[network], request_kwargs={"proxies": {'https': "http://" + self.proxy, 'http': "http://" + self.proxy}}))

        self.w3 = Web3(Web3.HTTPProvider(RPC[network]))
        self.account = self.w3.eth.account.from_key(private_key)
        self.address = self.account.address
        self.scan = SCANS[CHAIN_NAMES[self.w3.eth.chain_id]]

    def wait_balance(self, value, network):
        x = 0
        self.w3 = Web3(Web3.HTTPProvider(RPC[network]))
        while True:
    ...
    def check_allowance(self, token_address, contract_address):
        ...
        contract = self.w3.eth.contract(address=token_address, abi=ERC_20ABI)

...
from settings import stay_eth, amount, symbolWithdraw, decimal_places, transfer_subaccount, API
...
def okx_withdraw(address, amount_to_withdrawal, wallet_number, network):
    exchange = ccxt.okx({
        'apiKey': API.okx_apikey,
        'secret': API.okx_apisecret,
        'password': API.okx_passphrase,
...
def choose_cex(address, amount_to_withdrawal, wallet_number, network):
    if switch_cex == "okx":
        okx_withdraw(address, amount_to_withdrawal, wallet_number, network)
